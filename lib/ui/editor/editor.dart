...
import 'package:editor/model/syntax/syntax_definition.dart';
import 'package:editor/ui/design/custom_iconbutton.dart';
import 'package:editor/ui/editor/controller.dart';
import 'package:flutter/material.dart' hide Builder;
import 'package:flutter/services.dart';
// ignore: implementation_imports
import 'package:flutter/src/gestures/events.dart';
import 'package:editor/model/hid/key_codes.dart' as key_codes;
...
class _EditorState extends State<Editor> {
  ...
  late StreamSubscription _changeActiveFileSubscription;

  //List<CompletionItem> completions = [];

  late Map<Type, Action<Intent>> _actionMap;
  KeyMessageHandler? _storedFocusEventHandler;

  @override
  void initState() {
    super.initState();
    final dir = Project.of(context).fileSystem.directory('/editor/syntax');
    () async {
      final sx = dir.list();
      await for (final s in sx) {
        if (s.path.endsWith('.syntax') && (await s.stat()).type == FileSystemEntityType.file) {
          syntaxes.add(
              SyntaxDefinition.parseYaml(loadYaml(await Project.of(context).fileSystem.file(s.path).readAsString())));
        }
      }
      setState(() {
        currentSyntax = syntaxes.isEmpty ? null : syntaxes.first;
        final fn = Project.of(context).openFiles[_curFile]!.fileName;
    ...
    _focusNode.addListener(() {
      ...
      if (_focusNode.hasFocus) {
        _storedFocusEventHandler = ServicesBinding.instance.keyEventManager.keyMessageHandler;
        ServicesBinding.instance.keyEventManager.keyMessageHandler = (event) {
          final rawEvent = event.rawEvent;
          if (rawEvent == null) {
            return false;
          }
          final handled = onKeyEvent(rawEvent);
        ...
        if (_storedFocusEventHandler != null) {
          ServicesBinding.instance.keyEventManager.keyMessageHandler = _storedFocusEventHandler;
        }
      }
    });
    reupBlink();
    ...
    _actionMap = <Type, Action<Intent>>{
      NextFocusIntent: CallbackAction<NextFocusIntent>(
        onInvoke: (NextFocusIntent intent) => debugPrint('nextFocus'),
      ),
    };
  }
      ...
      if (openFiles.isNotEmpty) {
        ...
        if (_curFile != _displayedDoc) {
          ...
          if (currentSyntax != null) {
            ...
            lines = PlaintextSyntaxHighlighter().highlight(doc.map((it) => it['s']).toList().cast<String>());
          }
                          ...
                          child: Listener(
                            ...
                            child: ClipRect(
                              ...
                              child: CustomPaint(
                                ...
                                painter: EditorUi(-(controller.scrollPosition),
                                    theme: testTheme,
                                    lines: lines,
                                    lineHeight: 16.0,
                                    callback: () {},
  ...
  /*double _findCStart() {
    ...
    for (var cc in completions) {
      if (cc.textEdit != null) {
        return cc.textEdit.map((insert) => insert.range.start.character, (replace) => replace.insert.start.character) *
                8.2 +
            45;
      }
    }
  ...
  void cut() {
    _copy();
    setState(() {
      didUpdateDoc = controller.deleteSelection();
    });
  }
  ...
  bool onKeyEvent(RawKeyEvent event) {
    ...
    switch (event.data.runtimeType) {
      case RawKeyEventDataWeb:
        ...
        keyCode = key_codes.webMappings.containsKey(data.keyCode) ? key_codes.webMappings[data.keyCode]! : data.keyCode;
        isShift = data.isShiftPressed;
        characters = {48: ')'}[keyCode] ?? event.character ?? '';

        if (data.isControlPressed) {
          switch (characters) {
            case 's':
              saveCurrentFile();
              break;
      ...
      case RawKeyEventDataMacOs:
        ...
        isShift = data.isModifierPressed(ModifierKey.shiftModifier);
      ...
      setState(() {
        ...
        if (keyCode == key_codes.left) {
          ...
          if (cur.position != cur.endPosition) {
            moveCursor(cur.copyWith(
                line: cur.lastLine, endLine: cur.lastLine, position: cur.lastPosition, endPosition: cur.lastPosition));
          ...
          if (keyCode == key_codes.backspace) {
            ...
            if (isShift) {
              ...
              !controller.primaryCursor.isSelection) {
            final istart = lines[cur.line].indentStart();
            if (lines[cur.line].stringVal().trim().isEmpty && istart >= controller.file.indentData.amount) {
              final cur = controller.primaryCursor.copyWithSingle(position: istart);
              didUpdateDoc = controller.removeLeftAtAndInsert(cur, controller.file.indentData.amount, characters);
            } else {
              didUpdateDoc = controller.insert(characters);
            }
          ...
          if (shouldComplete && _curFile.endsWith('.dart')) {
            codeComplete(characters);

            if (!didComplete) {
              _suggestionStart =
                  lines[controller.primaryCursor.firstLine].suggestionStart(controller.primaryCursor.firstPosition - 1);
              completeLen = 0;
            }
            completeLen++;
            didComplete = true;
...
class EditorFilesHeader extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    var _find = 0;
    return Material(
      color: (_focusNode.hasFocus || EditorCursorUtil.cursors.isNotEmpty)
              ...
              child: ConstrainedBox(
                ...
                child: Row(
                  children: <Widget>[
                    ...
                    Padding(
                      padding: const EdgeInsets.only(top: 4.0, bottom: 4.0, left: 2.0, right: 3.0),
                      child: Text(
                        file.fileName,
