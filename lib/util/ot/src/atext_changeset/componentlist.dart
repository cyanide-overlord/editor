...
class ComponentList extends DirtyList<OpComponent> {
  ...
  ComponentList.from(Iterable<OpComponent> other, {bool isDirty: true, bool growable: true})
      : super.from(other, growable: growable) {
    _dirty = isDirty;
  }
  ...
  _unpack(AString astr, List pool) {
    ...
    var ops = _opsRegex.allMatches(astr.atts).map((m) {
      ...
      return OpComponent(opcode, chars, lines, attribs, charBank);
    });
   ...
   */
  AString toAString([List pool = const []]) {
