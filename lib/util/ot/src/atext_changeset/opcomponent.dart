...
class OpComponent {
  ...
  OpComponent sliceRight(int N, int L) {
    ...
    return OpComponent(opcode, chars - N, lines - L, attribs, isKeep ? charBank : charBank.substring(N));
  }

  OpComponent composeAttributes(AttributeList otherAtt) {
    return OpComponent(opcode, chars, lines, attribs.compose(otherAtt, isComposition: isKeep), charBank);
  }

  OpComponent transformAttributes(AttributeList otherAtt) {
    return OpComponent(opcode, chars, lines, attribs.transform(otherAtt), charBank);
  }

  OpComponent formatAttributes(AttributeList formatAtt) {
    return OpComponent(opcode, chars, lines, attribs.format(formatAtt), charBank);
  }

  OpComponent invertAttributes([AttributeList? exceptAtt]) {
    return OpComponent(opcode, chars, lines, attribs.invert(exceptAtt), charBank);
  }
  ...
  bool operator ==(other) => equalsButOpcode(other) && opcode == (other as OpComponent).opcode;
