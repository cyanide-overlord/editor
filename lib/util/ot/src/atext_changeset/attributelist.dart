...
class AttributeList extends UnmodifiableListView<OpAttribute> {
  ...
  factory AttributeList.fromMap({Map? format, Map? remove}) {
    ...
    var list = [
      for (var key in remove?.keys ?? []) OpAttribute.remove(key, remove![key]),
      for (var key in format?.keys ?? []) OpAttribute.format(key, format![key]),
    ];
    return AttributeList.from(list);
  }
  ...
  OpAttribute? find(String key) {
    try {
      return firstWhere((att) => att.key == key);
    } on StateError {
      return null;
    }
  }
      ...
      for (var j = 0; !skip && j < otherAtts.length; j++) {
        ...
        if (thisOp == otherOp) {
          ...
          if (thisOp.value.compareTo(otherOp.value) < 0) {
            ...
            res
              ..add(new OpAttribute.remove(otherOp.key, otherOp.value))
              ..add(thisOp);
          }
          skip = true;
    ...
    for (var i = 0; i < formatAtts.length; i++) {
      ...
      for (var j = 0; !skip && j < length; j++) {
        ...
        if (formatOp.key == thisOp.key && formatOp.value == thisOp.value) {
          ...
          res
            ..add(new OpAttribute.remove(thisOp.key, thisOp.value))
            ..add(formatOp);
          skip = true;
        }
      }
