...
class OT_text0 extends OTTypeFactory<String, List<Map>> with _BootstrapTransform<Map> {
  ...
  checkValidOp(List op) {
    op.forEach((o) => _checkValidComponent(o as Map));
  }
  ...
  append(List newOp, Map mc) {
    ...
    if ((c.hasInsert && c.insert.isEmpty) || (c.hasDelete && c.delete.isEmpty)) return;
  ...
  @override
  List<Map> compose(List op1, List op2) {
    checkValidOp(op1);
    checkValidOp(op2);
    var newOp = new List<Map>.from(op1.cast());
  ...
  // cancel one another.
  @override
  List<Map> invert(List op) => op.reversed.map((o) => _invertComponent(o)).toList();
}
