...
import 'package:editor/ui/editor/editor_ui.dart';
import 'package:editor/ui/editor/util/compose.dart';
import 'package:editor/util/ot/atext_changeset.dart';
import 'package:editor/util/patterns.dart';
...
class EditorController {
  ...
  String? get selectedText {
    ...
    for (var i = _primaryCursor.firstLine; i <= _primaryCursor.lastLine; i++) {
      ...
      } else if (i == _primaryCursor.lastLine) {
        line = line.substring(0, _primaryCursor.lastPosition);
      }
      data.write(line);
    }
  ...
  bool removeLeftAtAndInsert(EditorCursor cursor, int deleteSize, String text) {
    assert(!cursor.isSelection, 'Cursor cannot be a selection in removeLeft()');
    assert(!text.contains('\n'), 'Cannot have newlines in text in removeLeftAtAndInsert()');
    if (cursor.line == 0 && cursor.position == 0) return false;
    final cs = _beginChange();
    final startOfLine = cursor.position == 0;
    assert(!startOfLine || deleteSize == 1, 'Cannot delete more than 1 character at start of line with removeLeft()');
    ...
    cs.remove(deleteSize, startOfLine ? 1 : 0);
    if (startOfLine) {
      file.lineLengths[cursor.line - 1] += file.lineLengths[cursor.line] - 1 + text.length;
      file.lineLengths.removeAt(cursor.line);
    } else {
      file.lineLengths[cursor.line] -= deleteSize;
      file.lineLengths[cursor.line] += text.length;
    }
    cs.insert(text);
    _updateDoc(_finishChange(cs, primaryCursor, false), EditOpType.insert);
    return true;
  }
  ...
  EditorCursor cursorFromOffset(int start, int end) {
    var len = 0;
    var i = 0;
    int startLine = 0, endLine = 0, startPos = 0, endPos = 0, lastLen = 0;
