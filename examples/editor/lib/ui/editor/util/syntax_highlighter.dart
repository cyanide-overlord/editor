...
class StandardSyntaxHighlighter implements SyntaxHighlighter {
  ...
  late ListQueue<DefinitionContext> contexts;
    ...
    for (final line in lines) {
      ...
      do {
        ...
        if ((_pat.pop == null || !_pat.pop) && _pat.push != null) {
          _pat.push!.lookupOrGet(def.contexts).forEach((c) {
            if (c != null) contexts.addFirst(c);
          });
...
class PlaintextSyntaxHighlighter implements SyntaxHighlighter {
  @override
  List<EditorUiLine> highlight(List<String> lines) {
    return lines.map((e) => EditorUiLine([EditorTextFragment('undef', 'undef', e)])).toList();
  }
}
