...
class Changeset extends ComponentList {
  ...
  Changeset compose(Changeset otherCS) {
    ...
    if (newCs._newLen != otherCS._newLen) {
      throw new Exception(
          'new changeset length (${newCs._newLen}) does not match expected length (${otherCS._newLen})');
    }
    return newCs;
  }
  ...
  ADocument applyTo(ADocument doc) {
    if (_oldLen != doc.getLength()) {
      throw new Exception(
          'Trying to apply to a wrong document version, expected start length $_oldLen, got ${doc.getLength()}');
    }
  ...
  Map pack([List? pool]) {
    ...
    var op = 'X:' +
        _util.toString36(_oldLen) +
        (packed.dLen! >= 0 ? '>' : '<') +
        _util.toString36(packed.dLen!.abs()) +
        packed.atts;
